...
/** Katalogning o'ng paneli — tanlangan kategoriyaning 2 ustunli subkategoriya gridi */
export default function SubcategoryGrid({
  ...
  // Birinchi kartochka — butun kategoriya (subkategoriyasiz kategoriyalar uchun ham yagona yo'l)
  const tiles: Tile[] = [
    {
      key: `${category.id}-all`,
      name: 'Все модели',
  ...
  return (
    <div className={cn('overflow-y-auto px-3 pb-6 pt-3', className)}>
          ...
          >
            <div
              className={cn(
                ...
                tileItem.selected && 'ring-2 ring-primary ring-offset-2 ring-offset-background',
              )}
            >
              {tileItem.image && (
                <img
            ...
            <span
              className={cn(
                ...
                tileItem.selected ? 'font-semibold text-primary' : 'font-medium text-foreground',
              )}
            >
              {tileItem.name}
            </span>
